from national_agentic_ai_hackathon_2025_backend.database.hospital import HealthFacilityDB
...
async def main():
    ...
    # Retrieve all health facilities
    facilities_result = await db.get_all_health_facility()
    ...
    print(f"[green]Fetched {len(facilities)} health facilities.[/green]")
    print(facilities)
    # Create vector store with the fetched facilities
    data = vsm.create_vector_store("health_facility")
    ...
    print(data)
    print("[blue]Vector store 'health_facility' created with facility data.[/blue]")
