...
const iconMap = {
  ...
  Heart,
  Truck,
} satisfies Record<string, ComponentType<{ className?: string; style?: React.CSSProperties }>>
...
export default function Hero({setToggleChat}: {setToggleChat: React.Dispatch<React.SetStateAction<boolean>>}) {
  return (
    <section id="home" className="relative min-h-screen flex items-center justify-center px-6 overflow-hidden py-24">
      <div className="absolute inset-0 overflow-hidden pointer-events-none">
        <div className="absolute top-1/4 left-1/4 w-96 h-96 rounded-full blur-3xl bg-emerald-500/20" />
        <div className="absolute bottom-1/3 right-1/4 w-[450px] h-[450px] rounded-full blur-3xl bg-purple-500/20" />
      </div>
      ...
      <div className="relative max-w-6xl mx-auto text-center z-10">
        ...
        <div className="flex flex-col sm:flex-row gap-4 justify-center items-center mb-16">
          <Button
            size="lg"
            className="px-8 py-4 rounded-xl text-lg font-bold shadow-lg transition-all bg-gradient-to-r from-emerald-600 to-teal-600 hover:from-emerald-500 hover:to-teal-500 text-white flex items-center gap-3 hover:cursor-pointer"
          ...
          <Button
            size="lg"
            className="px-8 py-4 rounded-xl text-lg font-semibold backdrop-blur-sm transition-all flex items-center gap-3 bg-slate-800/50 hover:bg-slate-800/70 text-slate-100 border border-slate-700 hover:cursor-pointer"
        ...
        <div className="grid grid-cols-2 lg:grid-cols-4 gap-4 max-w-4xl mx-auto">
          {services.map((service) => {
            const Icon = iconMap[service.icon] as ComponentType<{ className?: string; style?: React.CSSProperties }>
            return (
              <button
                key={service.id}
              ...
              >
                <div
                  className={`w-12 h-12 mx-auto mb-3 rounded-xl flex items-center justify-center bg-gradient-to-br ${
